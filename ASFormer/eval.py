...
def func_eval(dataset, recog_path, file_list, data_dir):
    ground_truth_path = data_dir + dataset + "/groundTruth/"
    mapping_file = data_dir + dataset + "/mapping.txt"
...
def main():
    ...
    if args.split == 0:
        for split in range(1, cnt_split_dict[args.dataset] + 1):
            ...
            file_list = args.data_dir+args.dataset+"/splits/test.split{}".format(split)+".bundle"
            acc, edit, f1s = func_eval(args.dataset, recog_path, file_list, args.data_dir)
            acc_all += acc
            edit_all += edit
            f1s_all[0] += f1s[0]
            f1s_all[1] += f1s[1]
        ...
        recog_path = "./{}/".format(args.result_dir)+args.dataset+"/split_{}".format(split)+"/"
        file_list = args.data_dir+args.dataset+"/splits/test.split{}".format(split)+".bundle"
        acc_all, edit_all, f1s_all = func_eval(args.dataset, recog_path, file_list, args.data_dir)
